...
export default function SingUpPage() {
  ...
  function Header() {
    ...
    return (
      ...
      <>
        <header className={` flex flex-row   bg-gradient-to-r from-sky-800 to-indigo-800 w-12/12 h-[58px] sticky top-0 z-40  ${width <= 500 ? "w-[120%] "  : "w-12/12 " }`}>
  ...
  const SingUping = () => {
    if((password.length < 8) || (password2.length < 8)){
      console.log("Password is small , input bigger password at least 8 symbols")
    }
