...
export default function HomePage() {
    ...
    return (
      ...
      <>
        <header className={` flex flex-row   bg-gradient-to-r from-sky-800 to-indigo-800 w-12/12 h-[58px] sticky top-0 z-40 `}>
          ...
          <button className={` ${isThemeChanged ? 'bg-gray-700' : 'bg-zinc-200'}  z-50 w-[4%] h-[30px] ${width > 500 ? `fixed top-[13px] ` : 'relative bottom-[43px]  '} outline-none rounded-xl ${isWide ? 'min-w-14' : 'min-w-[58px]'} ${isWide ? 'left-[52%]' : 'left-[61%]'}`} suppressHydrationWarning={true} onClick={themeChange}></button>
          <button className={` ${isThemeChanged ? 'bg-zinc-200' : 'bg-black'} z-50 w-[1.4%] h-[25px] ${width > 500 ? 'fixed top-[15px]' : 'hidden'}   rounded-[50px] ${themeChanger} min-w-6`} suppressHydrationWarning={true} onClick={themeChange}></button>
          <button className={` ${isThemeChanged ? 'bg-zinc-200' : 'bg-black'} z-50 w-[1.4%] h-[25px] ${width > 500 ? 'hidden' : 'block '} relative bottom-[77px] right-[22%] rounded-[50px] ${themeChanger} min-w-6`} suppressHydrationWarning={true} onClick={themeChange}></button>
  ...
  return (
    ...
    <div className="h-full min-h-[1000px]  m-0 p-0 overflow-x-hidden">
      ...
      <div className={`w-[26px] h-[26px] bg-lime-600 rounded-xl z-50 fixed top-[5px] ${width <= 500 ? "w-[120%] " : ""} left-[85.8%] ${totalCounter > 0 ? 'block' : 'hidden'} text-center text-zinc-200 ${isWide ? 'block' : ' hidden'}`}>{totalCounter}</div>
      <div className={` ${isThemeChanged ? 'bg-gray-900' : 'bg-gray-300'} ${width > 0 ? 'w-12/12' : 'w-[100%]'}  ${width > 0 ? '' : 'bottom-[110px]'} relative bg-no-repeat bg-cover scroll-smooth  ${InfoWindowCounter > 0 ? ' bottom-[20px] h-[1250px] ' : ''} `}>

        <div className={` z-30 sticky top-[56px] w-12/12  bg-cyan-700   m-0 p-0 overflow-x-hidden text-2xl text-zinc-200 flex flex-row flex-wrap justify-center items-center ${InfoWindowCounter > 0 ? 'hidden' : 'block'} `}>
